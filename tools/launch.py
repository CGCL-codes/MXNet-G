...
def dmlc_opts(opts):
    ...
    args = ['--num-workers', str(opts.num_workers),
            ...
            '--dmlc-interface', opts.dmlc_interface, #yegeyan 2016.10.6
            '--parameter-partition-num', str(opts.parameter_partition_num), #yegeyan 2016.12.9
...
def main():
    ...
    parser.add_argument('-i', '--dmlc-interface', type=str, default='eth0',
                        help = 'the network interface to use') #yegeyan 2016.10.6
    parser.add_argument('-p', '--parameter-partition-num', type=int, default=-2,
                        help = 'the parameter partition num in a server, if p == 0, then not use this strategy') #yegeyan 2016.12.9
